...
class PriceWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun updateAllWidgets(
            ...
            val dfNormal = DecimalFormat("#,##0.0000")
            val dfMetal = DecimalFormat("#,##0.00")
            ...
            for (widgetId in appWidgetIds) {
                ...
                if (activeSymbols.isNotEmpty()) {
                    ...
                    val isMetal = sym.contains("XAU") || sym.contains("XAG") || sym.contains("JPY")
                    val df = if (isMetal) dfMetal else dfNormal
                    
                    views.setViewVisibility(R.id.row_symbol_1, View.VISIBLE)
                    views.setTextViewText(R.id.text_symbol_1_name, sym)
                    if (tick != null) {
                        views.setTextViewText(R.id.text_symbol_1_price, df.format(tick.price))
                ...
                if (activeSymbols.size > 1) {
                    ...
                    if (tick != null) {
                        views.setTextViewText(R.id.text_symbol_2_price, df.format(tick.price))
                        val sign = if (tick.change >= 0) "+" else ""
                ...
                if (activeSymbols.size > 2) {
                    ...
                    if (tick != null) {
                        views.setTextViewText(R.id.text_symbol_3_price, df.format(tick.price))
                        val sign = if (tick.change >= 0) "+" else ""
